...
#define STDGPU_DETAIL_COMPOUND_BINARY_OPERATOR(NAME, OP, RETURN_TYPE)                                                  \
    ...
    inline STDGPU_HOST_DEVICE auto NAME<void>::operator()(T&& lhs, U&& rhs)                                            \
            const->decltype(stdgpu::forward<T>(lhs) OP stdgpu::forward<U>(rhs))                                                        \
    {                                                                                                                  \
        return stdgpu::forward<T>(lhs) OP stdgpu::forward<U>(rhs);                                                                     \
    }
...
bit_not<void>::operator()(T&& value) const -> decltype(~stdgpu::forward<T>(value))
{
    return ~stdgpu::forward<T>(value);
}
