...
def get_realsense_remappings(mode: NvbloxMode) -> List[Tuple[str, str]]:
    # 临时修改：为 Orbbec 相机使用直接的话题映射
    remappings = []
    remappings.append(('camera_0/depth/image', '/camera/depth/image_raw'))  # 直接使用 Orbbec 话题
...
def get_orbbec_remappings(mode: NvbloxMode) -> List[Tuple[str, str]]:
    remappings = []
    remappings.append(('camera_0/depth/image', '/camera/depth/image_raw'))  # 修正：使用 image_raw 而不是 image_rect_raw
    remappings.append(('camera_0/depth/camera_info', '/camera/depth/camera_info'))
    if mode is NvbloxMode.people:
        remappings.append((f'mask/image', '/unet/raw_segmentation_mask'))
        remappings.append((f'mask/camera_info', '/segmentation/camera_info_resized'))
...
# 为了兼容性，我们重写 realsense 映射来支持 Orbbec
def get_realsense_remappings_for_orbbec(mode: NvbloxMode) -> List[Tuple[str, str]]:
    # 使用 Orbbec 的话题映射，但保持函数名为 realsense 以兼容现有代码
    return get_orbbec_remappings(mode)
...
def add_nvblox(args: lu.ArgumentContainer) -> List[Action]:
    ...
    if camera is NvbloxCamera.isaac_sim:
        ...
        camera_config = realsense_config  # Use realsense config as base for Orbbec
